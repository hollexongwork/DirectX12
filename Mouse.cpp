...
void clipToWindow(void)
{
    ...
    float clipSize = 100.0f;

    POINT ul;
    ul.x = rect.left + clipSize;
    ul.y = rect.top + clipSize;

    POINT lr;
    lr.x = rect.right - clipSize;
    lr.y = rect.bottom - clipSize;
...
{
    return gState.leftButton && !gPrevState.leftButton;
}
...
{
    return (float)gState.x * 0.01f;
}

float MousePositionY(void)
{
    return (float)gState.y * 0.01f;
}
...
Mouse_PositionMode GetMouseMod(void)
{
    return gMode;
}
